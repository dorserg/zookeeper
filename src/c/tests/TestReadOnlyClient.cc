...
public:
    ...
    void startPeer() {
        char cmd[1024];
        sprintf(cmd, "%s startPeer", ZKSERVER_CMD);
    ...
    void testReadOnly() {
        startPeer();
        watchctx_t watch;
        zhandle_t* zh = zookeeper_init_ro(
                "localhost:22181", watcher, 10000, 0, &watch, 1, 0);
        watch.zh = zh;
        CPPUNIT_ASSERT(zh!=0);
        sleep(1);
        int len = 1024;
        ...
        char path[1024];
        res = zoo_create(zh, "/test", buf, 10, &ZOO_OPEN_ACL_UNSAFE, 0, path, 512);
